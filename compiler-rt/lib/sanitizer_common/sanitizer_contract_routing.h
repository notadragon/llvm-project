...
//
// This exists so that asan, ubsan, tsan and msan do not each spell all three
// for themselves.  Spelled per-sanitizer, the enum is written out four times,
// the populator struct four times under four names with identical layout,
// and -- worst -- the one `extern "C"` entry point is declared with four
// different prototypes, one per sanitizer's own populator type.  Being
// extern "C" the names do not mangle, so nothing diagnoses that, and libasan
// links libubsan, which would put two of those declarations in a single
// shared object.
//
