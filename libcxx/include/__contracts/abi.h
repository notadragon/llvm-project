...
struct __cxa_descriptor_table_t {
  ...
  __UINT8_TYPE__
  table_version() const { return header >> 4; }

  __UINT8_TYPE__
  vendor_id() const { return header & 0x0F; }

  const __cxa_descriptor_data_t* data() const {
    auto __p = reinterpret_cast<__UINTPTR_TYPE__>(field_ids + num_entries);
    __p      = (__p + alignof(__cxa_descriptor_data_t) - 1) & ~(alignof(__cxa_descriptor_data_t) - 1);
...
using __cxa_local_handler_fn_t = int (*)(const void* /*label_ptr*/, const void* /*contract_violation_ptr*/);

// Query function pointer type.
using __cxa_query_fn_t = void* (*)(const void* /*label_ptr*/, const void* /*key*/, std::size_t /*index*/);
...
extern "C" const void*
__cxa_find_field(const __cxa_contract_data_block* __chain, const __UINT8_TYPE__* __ids, __UINT8_TYPE__ __num_ids);

// Typed convenience: find a scalar value field.
template <typename _Tp>
_Tp __cxa_find_field_value(const __cxa_contract_data_block* __chain, __UINT8_TYPE__ __field_id, _Tp __default_value) {
...
template <typename _Tp>
const _Tp* __cxa_find_field_ptr(const __cxa_contract_data_block* __chain, __UINT8_TYPE__ __field_id) {
  __UINT8_TYPE__ __ids[] = {__field_id};
  return static_cast<const _Tp*>(__cxa_find_field(__chain, __ids, 1));
}
...
void __contract_dispatch_core(const __cxxabiv1::__cxa_contract_data_block* __chain, __UINT8_TYPE__ __semantic);
void __dispatch_with_override_core(
    void* __data, __UINT8_TYPE__ __kind, __UINT8_TYPE__ __semantic, __UINT8_TYPE__ __mode);
...
[[noreturn]] void __c_contract_check_enforce(
    const char* __comment, const char* __file, const char* __func, unsigned __line, unsigned char __kind);
...
void __c_contract_check_noexcept(
    ...
    const char* __func,
    unsigned __line,
    unsigned char __kind,
    unsigned char __semantic);
...
void __contract_dispatch_core_noexcept(const __cxxabiv1::__cxa_contract_data_block* __chain,
