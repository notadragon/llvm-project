...
// accessor.
const char* std::contracts::contract_violation::report() const noexcept {
  auto __p = __cxa_find_field_ptr<__cxa_contract_report_populator>(__chain_, CXA_FIELD_REPORT);
  if (!__p || !__p->populate)
    return nullptr;
...
void __nx_override(void* __data, __UINT8_TYPE__ __kind, __UINT8_TYPE__ __semantic, __UINT8_TYPE__ __mode) {
#if _LIBCPP_HAS_EXCEPTIONS
  try {
...
extern "C" _LIBCPP_EXPORTED_FROM_ABI void __cxa_contract_violation_noexcept(void* __data) noexcept {
  auto* __chain = static_cast<const __cxa_contract_data_block*>(__data);
  __UINT8_TYPE__ __sem =
      __cxa_find_field_value<__UINT8_TYPE__>(__chain, CXA_FIELD_EVALUATION_SEMANTIC, CXA_ES_UNSPECIFIED);
#if _LIBCPP_HAS_EXCEPTIONS
  try {
...
__contract_dispatch_core_noexcept(const __cxa_contract_data_block* __chain, __UINT8_TYPE__ __semantic) noexcept {
#if _LIBCPP_HAS_EXCEPTIONS
  try {
...
#define CXA_OBSERVE_NX(kind_name, kind_val, mode_name, mode_val)                                                       \
  extern "C" _LIBCPP_EXPORTED_FROM_ABI void __cxa_contract_violation_##kind_name##_observe_##mode_name##_noexcept(     \
...
#define CXA_ENFORCE_NX(kind_name, kind_val, mode_name, mode_val)                                                       \
  ...
  __cxa_contract_violation_##kind_name##_enforce_##mode_name##_noexcept(void* __data) noexcept {                       \
...
#define CXA_NOEXCEPT_OBSERVE(kind_name, kind_val, mode_name, mode_val)                                                 \
  extern "C" _LIBCPP_EXPORTED_FROM_ABI void                                                                            \
  __cxa_contract_violation_##kind_name##_noexcept_observe_##mode_name##_noexcept(void* __data) noexcept {              \
...
#define CXA_NOEXCEPT_ENFORCE(kind_name, kind_val, mode_name, mode_val)                                                 \
  extern "C" [[noreturn]] _LIBCPP_EXPORTED_FROM_ABI void                                                               \
  __cxa_contract_violation_##kind_name##_noexcept_enforce_##mode_name##_noexcept(void* __data) noexcept {              \
...
extern "C" [[noreturn]] _LIBCPP_EXPORTED_FROM_ABI void __cxa_pure_virtual_quick(void) noexcept { __builtin_trap(); }
...
extern "C" [[noreturn]] _LIBCPP_EXPORTED_FROM_ABI void __cxa_pure_virtual_enforce(void) {
  __c_contract_check_enforce("pure virtual function called", "", "", 0, CXA_AK_IMPLICIT);
  __builtin_unreachable();
}
...
extern "C" [[noreturn]] _LIBCPP_EXPORTED_FROM_ABI void __cxa_pure_virtual_noexcept_enforce(void) noexcept {
  __c_contract_check_noexcept("pure virtual function called", "", "", 0, CXA_AK_IMPLICIT, CXA_ES_NOEXCEPT_ENFORCE);
  __builtin_unreachable();
}
...
extern "C" [[noreturn]] _LIBCPP_EXPORTED_FROM_ABI void __cxa_pure_virtual_observe(void) {
  __c_contract_check_observe("pure virtual function called", "", "", 0, CXA_AK_IMPLICIT);
  __builtin_abort();
}

// noexcept_observe: as observe, but a throwing handler terminates at the
// noexcept barrier.
extern "C" [[noreturn]] _LIBCPP_EXPORTED_FROM_ABI void __cxa_pure_virtual_noexcept_observe(void) noexcept {
  __c_contract_check_noexcept("pure virtual function called", "", "", 0, CXA_AK_IMPLICIT, CXA_ES_NOEXCEPT_OBSERVE);
