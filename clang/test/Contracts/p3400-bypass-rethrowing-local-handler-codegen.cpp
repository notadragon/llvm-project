...
//
// And again under -g, which is the interesting case: GCC loses this
// optimization to debug info, its walk treating DEBUG_BEGIN_STMT as an
// unanalysable statement, and a suite that compiles without -g never sees it
// while Compiler Explorer and most real builds always pass it.  Clang walks
// the AST and so has no such hole; this pins it.  GCC mirror:
