// P3100 x P4298: misaligned access configured to "noexcept_observe": reports
// (sem=6) then proceeds.
// RUN: %clangxx -std=c++26 %s -fcontracts -fcontracts-p3100 -fcontracts-p4298 \
// RUN:   -fcontract-configuration-file=%S/p3100-align-noexcept-observe.json \
...
// CHECK: VIOL kind=7 sem=6 comment=[misaligned pointer access]
// CHECK: RESULT=
