...
// from.  InstantiateContractSpecifier's idempotence guard must consider both:
// comparing the placeholder against only the latter concludes it has already
// been substituted and returns, leaving the instantiation holding a dependent
// predicate that references the *pattern's* ParmVarDecls.  CodeGen then hits
// whichever assertion the predicate's contents reach first -- a parameter
// reference gives EmitDeclRefLValue's "Should not use decl without marking it
// used", `sizeof(T)` gives CodeGenTypes' placeholder-type unreachable, a
// member call gives "Call must have function pointer type", and so on.  One
