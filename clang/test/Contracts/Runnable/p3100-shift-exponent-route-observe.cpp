// P3100 full-coverage UBSan routing (Clang): the shift-exponent check routes to the
...
// RUN:   -fsanitize=shift-exponent -fsanitize-semantic=shift-exponent:noexcept_observe %libcxx_flags \
...
int main() { volatile int a = 1, b = 40; volatile int r = a << b; (void)r; std::printf("survived\n"); std::fflush(stdout); }
