...
//
// CodeGen must pass one shared global data block to both entry points, as GCC
// does.  Building the extended block -- the one carrying the local handler and
// query pointers -- only on the predicate-false path, and passing the *basic*
// block to the _ex entry point, means a throwing predicate never calls the
// local handler and query_control_object silently answers null, for every
// assertion kind.
//
