// P3100 full-coverage UBSan routing (Clang): the bounds check routes to the
...
// RUN:   -fsanitize=bounds -fsanitize-semantic=bounds:noexcept_observe %libcxx_flags \
...
int main() { int a[3] = {}; volatile int i = 5; volatile int r = a[i]; (void)r; std::printf("survived\n"); std::fflush(stdout); }
