// P3100 full-coverage UBSan routing (Clang): the builtin check routes to the
...
// RUN:   -fsanitize=builtin -fsanitize-semantic=builtin:noexcept_observe %libcxx_flags \
...
int main() { volatile unsigned z = 0; volatile int r = __builtin_ctz(z); (void)r; std::printf("survived\n"); std::fflush(stdout); }
