...
//
// The hazard is general to any declaration/definition split: if
// RebuildContractSpecifierForDecl copies the capture to the definition
// without its initializer, the postcondition reads 'old' as uninitialized
// storage and a spurious violation fires on the qualified call.  The capture
// initializer has to be rebuilt against the definition's parameters.
