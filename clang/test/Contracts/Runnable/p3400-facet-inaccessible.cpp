...
// program.
//
// Getting this wrong turns every case below into a hard error reported
// against the contract, for a subtle reason: contracts are parsed as part of
// a declarator, where Clang *delays* access checking so it can be redone once
// the declaration's context is known.  A probe run inside an SFINAE trap
// therefore sees no access failure at all -- the check is not skipped, it is
// parked, and it fires later, after the trap is gone.  Detection drops
// inaccessible candidates before probing, so no such check is ever queued.
//
