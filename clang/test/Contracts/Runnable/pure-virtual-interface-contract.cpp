...
//
// A compiler that substitutes contracts only when it instantiates a
// definition cannot compile this at all, and one that has just gained
// on-odr-use substitution still needs to be shown that the checks it gained
// actually run.  This file is that proof, on both compilers.
//
// The predicate is deliberately non-constant but NOT value-dependent: a
// dependent one reaches CodeGen still dependent and asserts there (see
