...
// and rejects a non-const one.  There is a way to get each side wrong:
//
//   - A static `query' crashes the compiler if detection calls
...
//     is an ordinary function lvalue, so the assertion fires.  Detection goes
//     through BuildCallExpr, which dispatches on the callee's real form.
//
//   - A non-const `handle_contract_violation' is accepted as a facet if
//     detection only forms a member reference, which succeeds for a member of
//     any signature and, on a const object, succeeds even for a non-const
//     member: the constness of the implicit object argument is not checked
//     until a call is built.  The concept says such a member is not a facet,
//     so detection builds the call the concept describes.
//
// Combining a label makes both louder, since __combined_label dispatches on
// the concepts: a disagreement shows up as the bare label and the combined
// one answering differently for the same type.  The last section pins that
// they agree.
...
// This is the shape that crashes the compiler outright if detection builds
// the call as a member call.
