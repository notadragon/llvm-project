// P3100: misaligned access configured to "ignore": no defined substitute for the
// lvalue, so ignore is the raw access (no handler, no check).
// RUN: %clangxx -std=c++26 %s -fcontracts -fcontracts-p3100 \
// RUN:   -fcontract-configuration-file=%S/p3100-align-ignore.json \
