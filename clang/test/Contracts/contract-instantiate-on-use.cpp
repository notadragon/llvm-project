...
// got it right; the file exists because GCC did not.  Deferring contract
// substitution to instantiate_body, as GCC did, leaves a function that is
// never defined with its contracts never substituted at all, and its P3097
// interface wrapper walks the still-dependent predicate into an ICE.  GCC now
// instantiates on odr-use too; this side pins the behaviour so the two suites
// ask the same question.
//
// `pre(true)` would not be a regression test for it: a predicate that is
// already a constant needs no substitution to be usable, which is how such a
// defect stays hidden.  Every predicate below contains something substitution
// has to rewrite.
//
