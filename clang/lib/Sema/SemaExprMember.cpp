...
Sema::BuildImplicitMemberExpr(const CXXScopeSpec &SS,
  ...
  // [over.call.func]/p3.1 and [expr.prim.id.general]: an unqualified reference
  // to a non-static member -- which forms an implied 'this' access here -- is
  ...
  if (const ContractScopeRecord *CSR = getCurrentContractEntry()) {
    ...
    bool CtorPre = CSR->Kind == ContractKind::Pre &&
                   isa_and_nonnull<CXXConstructorDecl>(FnDC);
    bool DtorPost = CSR->Kind == ContractKind::Post &&
                    isa_and_nonnull<CXXDestructorDecl>(FnDC);
    if (CtorPre || DtorPost) {
