//===- dispatch.c - libcontracts contract-violation dispatch --------------===//
//
...
static const void *find_field1(const __cxa_contract_data_block *chain,
                               uint8_t id) {
  uint8_t ids[1];
  ids[0] = id;
...
static void __contract_minimal_default(const __cxa_contract_data_block *chain) {
  ...
  fprintf(stderr, "contract violation in function %s at %s:%u: %s\n", func,
          file, line, comment);
}
...
#define CXA_OBSERVE(kn, kv, mn, mv)                                            \
  void __cxa_contract_violation_##kn##_observe_##mn(void *data) {              \
...
#define CXA_ENFORCE(kn, kv, mn, mv)                                            \
  void __cxa_contract_violation_##kn##_enforce_##mn(void *data) {              \
